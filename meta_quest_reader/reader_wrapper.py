#!/usr/bin/env python3
"""Meta Quest Transform Handler - Core transform logic without ROS2 dependencies."""
...
class MetaQuestReaderWrapper:
    """
    Wrapper class for the MetaQuestReader class from RAIL Lab to handle the new APK created by Neuracore.

    The class provides clean APIs to access the hand controller transforms in the OpenXR coordinate system 
    and ROS coordinate system, and button event callbacks.
    """
    ...
    def get_hand_controller_transform_openxr(
        ...
        """Get the 4x4 transformation matrix for a hand controller in the OpenXR coordinate system.
        
        Args:
            hand: Which hand ('left', 'right', 'l', or 'r')
            transform_type: Type of transform to get:
                - 'grip': Grip pose on the handle (tilted, default for backwards compatibility)
                - 'pointer': Pointer/ray-cast pose (straight forward, recommended for control)
                - 'model': Device model pose
            
        Returns:
            4x4 numpy array transformation matrix, or None if not available
        """
        ...
        type_suffix = {
            ...
            'pointer': 'p',
            'model': 'm'
        }.get(transform_type, 'g')
        
        key = hand_key + type_suffix # e.g. 'rg' for right grip, 'lp' for left pointer
    ...
    def get_hand_controller_transform_ros(
        ...
        Returns:
            4x4 transformation matrix in ROS coordinates, or None if not available
        """
        transform_openxr = self.get_hand_controller_transform_openxr(hand, transform_type)
    ...
    def get_button_state(self, button_name: str) -> bool:
        ...
        Args:
            button_name: Button name (e.g., 'A', 'B', 'X', 'Y', 'RJ', 'LJ')
            
        Returns:
            True if button is pressed, False otherwise
        """
        # TODO: think about this. if the button was not found and we just return False.
        return self._latest_buttons.get(button_name, False)

    
    def get_grip_value(self, hand: Literal['left', 'right', 'l', 'r'] = 'right') -> float:
        ...
        Returns:
            Float value in range [0.0, 1.0] where 0.0 is not pressed and 1.0 is fully pressed
        """
        hand_key = self._normalize_hand_key(hand)
    ...
    def get_trigger_value(self, hand: Literal['left', 'right', 'l', 'r'] = 'right') -> float:
    ...
    def get_joystick_value(self, hand: Literal['left', 'right', 'l', 'r'] = 'right') -> tuple[float, float]:
    ...
    def _normalize_hand_key(self, hand: Literal['left', 'right', 'l', 'r']) -> str:
        ...
        if hand in ('left', 'l'):
            ...
            raise ValueError(f"Invalid hand: {hand}. Must be 'left', 'right', 'l', or 'r'")
